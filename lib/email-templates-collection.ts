export interface EmailTemplate {
  id: string
  name: string
  category: string
  description: string
  subject: string
  previewText: string
  icon: string
  color: string
}

export const emailTemplates: EmailTemplate[] = [
  // Transfer Notifications (1-5)
  {
    id: "transfer-received",
    name: "Transfer Received",
    category: "Transfer",
    description: "Notify recipient of incoming e-Transfer",
    subject: "You've received a Zelle payment for {{amount}}",
    previewText: "{{senderName}} sent you money",
    icon: "ArrowDownCircle",
    color: "from-green-500 to-emerald-600",
  },
  {
    id: "transfer-sent",
    name: "Transfer Sent Confirmation",
    category: "Transfer",
    description: "Confirm outgoing transfer to sender",
    subject: "Your Zelle payment of {{amount}} was sent",
    previewText: "Transfer to {{recipientName}} successful",
    icon: "ArrowUpCircle",
    color: "from-blue-500 to-blue-600",
  },
  {
    id: "transfer-pending",
    name: "Transfer Pending",
    category: "Transfer",
    description: "Notify about pending transfer status",
    subject: "Your e-Transfer is pending - Action required",
    previewText: "Complete your transfer within 30 days",
    icon: "Clock",
    color: "from-amber-500 to-orange-600",
  },
  {
    id: "transfer-cancelled",
    name: "Transfer Cancelled",
    category: "Transfer",
    description: "Notify about cancelled transfer",
    subject: "Your Zelle payment has been cancelled",
    previewText: "Transfer of {{amount}} was cancelled",
    icon: "XCircle",
    color: "from-red-500 to-red-600",
  },
  {
    id: "transfer-expired",
    name: "Transfer Expired",
    category: "Transfer",
    description: "Notify about expired transfer",
    subject: "Your Zelle payment has expired",
    previewText: "The transfer was not claimed in time",
    icon: "AlertTriangle",
    color: "from-gray-500 to-gray-600",
  },

  // Deposit Notifications (6-10)
  {
    id: "deposit-completed",
    name: "Deposit Completed",
    category: "Deposit",
    description: "Confirm successful deposit",
    subject: "Deposit of {{amount}} completed successfully",
    previewText: "Funds are now in your account",
    icon: "CheckCircle",
    color: "from-emerald-500 to-green-600",
  },
  {
    id: "deposit-failed",
    name: "Deposit Failed",
    category: "Deposit",
    description: "Notify about failed deposit attempt",
    subject: "Your e-Transfer deposit could not be completed",
    previewText: "Action required to complete deposit",
    icon: "XOctagon",
    color: "from-red-500 to-rose-600",
  },
  {
    id: "deposit-reminder",
    name: "Deposit Reminder",
    category: "Deposit",
    description: "Remind recipient to deposit funds",
    subject: "Reminder: You have an unclaimed e-Transfer",
    previewText: "Don't forget to deposit your {{amount}}",
    icon: "Bell",
    color: "from-yellow-500 to-amber-600",
  },
  {
    id: "deposit-instructions",
    name: "Deposit Instructions",
    category: "Deposit",
    description: "Step-by-step deposit guide",
    subject: "How to deposit your Zelle payment",
    previewText: "Follow these simple steps",
    icon: "FileText",
    color: "from-indigo-500 to-violet-600",
  },
  {
    id: "auto-deposit-enabled",
    name: "Auto-Deposit Enabled",
    category: "Deposit",
    description: "Confirm auto-deposit registration",
    subject: "Zelle Auto-Deposit is now active",
    previewText: "Transfers will deposit automatically",
    icon: "Zap",
    color: "from-cyan-500 to-teal-600",
  },

  // Security Notifications (11-15)
  {
    id: "security-question-updated",
    name: "Security Question Updated",
    category: "Security",
    description: "Confirm security question change",
    subject: "Your e-Transfer security settings were updated",
    previewText: "Security question successfully changed",
    icon: "Shield",
    color: "from-purple-500 to-purple-600",
  },
  {
    id: "suspicious-activity",
    name: "Suspicious Activity Alert",
    category: "Security",
    description: "Alert about unusual transfer activity",
    subject: "Important: Unusual activity detected",
    previewText: "Review your recent transactions",
    icon: "AlertOctagon",
    color: "from-red-600 to-red-700",
  },
  {
    id: "password-reset",
    name: "Password Reset",
    category: "Security",
    description: "Password reset instructions",
    subject: "Reset your password",
    previewText: "Click to reset your password",
    icon: "Key",
    color: "from-orange-500 to-red-600",
  },
  {
    id: "two-factor-enabled",
    name: "Two-Factor Authentication",
    category: "Security",
    description: "Confirm 2FA activation",
    subject: "Two-factor authentication is now enabled",
    previewText: "Your account is more secure",
    icon: "Lock",
    color: "from-green-600 to-emerald-700",
  },
  {
    id: "login-notification",
    name: "New Login Detected",
    category: "Security",
    description: "Alert about new device login",
    subject: "New sign-in to your account",
    previewText: "Was this you?",
    icon: "UserCheck",
    color: "from-blue-600 to-indigo-700",
  },

  // Account Notifications (16-19)
  {
    id: "account-verified",
    name: "Account Verified",
    category: "Account",
    description: "Confirm account verification",
    subject: "Your account has been verified",
    previewText: "You're all set to send and receive",
    icon: "BadgeCheck",
    color: "from-green-500 to-teal-600",
  },
  {
    id: "profile-updated",
    name: "Profile Updated",
    category: "Account",
    description: "Confirm profile changes",
    subject: "Your profile has been updated",
    previewText: "Changes saved successfully",
    icon: "UserCog",
    color: "from-slate-500 to-slate-600",
  },
  {
    id: "bank-linked",
    name: "Bank Account Linked",
    category: "Account",
    description: "Confirm bank connection",
    subject: "Bank account successfully linked",
    previewText: "You can now send and receive e-Transfers",
    icon: "Building",
    color: "from-blue-500 to-cyan-600",
  },
  {
    id: "limit-increase",
    name: "Transfer Limit Increased",
    category: "Account",
    description: "Notify about limit changes",
    subject: "Your e-Transfer limit has been increased",
    previewText: "You can now send up to {{limit}}",
    icon: "TrendingUp",
    color: "from-emerald-500 to-green-600",
  },

  // Request & Notification (20-22)
  {
    id: "money-request",
    name: "Money Request",
    category: "Request",
    description: "Request money from someone",
    subject: "{{senderName}} is requesting {{amount}}",
    previewText: "Tap to send the requested amount",
    icon: "HandCoins",
    color: "from-pink-500 to-rose-600",
  },
  {
    id: "request-accepted",
    name: "Request Accepted",
    category: "Request",
    description: "Notify that request was fulfilled",
    subject: "Your money request was accepted",
    previewText: "{{recipientName}} sent you {{amount}}",
    icon: "ThumbsUp",
    color: "from-green-500 to-emerald-600",
  },
  {
    id: "request-declined",
    name: "Request Declined",
    category: "Request",
    description: "Notify that request was declined",
    subject: "Your money request was declined",
    previewText: "{{recipientName}} couldn't fulfill your request",
    icon: "ThumbsDown",
    color: "from-gray-500 to-slate-600",
  },

  // New Templates (23-33)
  {
    id: "transfer-receipt",
    name: "Transfer Receipt",
    category: "Transfer",
    description: "Official receipt for a completed e-Transfer",
    subject: "Your Zelle payment receipt",
    previewText: "Save this receipt for your records",
    icon: "Receipt",
    color: "from-teal-500 to-cyan-600",
  },
  {
    id: "scheduled-transfer",
    name: "Scheduled Transfer",
    category: "Transfer",
    description: "Reminder for an upcoming scheduled transfer",
    subject: "Reminder: Scheduled Zelle payment tomorrow",
    previewText: "Ensure sufficient funds before the transfer date",
    icon: "CalendarClock",
    color: "from-sky-500 to-blue-600",
  },
  {
    id: "deposit-on-hold",
    name: "Deposit on Hold",
    category: "Deposit",
    description: "Notify recipient that their deposit is under review",
    subject: "Your deposit is temporarily on hold",
    previewText: "We are reviewing your deposit before releasing funds",
    icon: "PauseCircle",
    color: "from-amber-600 to-yellow-700",
  },
  {
    id: "two-factor-code",
    name: "Two-Factor Code",
    category: "Security",
    description: "One-time verification code for sign-in",
    subject: "Your one-time verification code",
    previewText: "Use this code to complete your sign-in",
    icon: "ScanLine",
    color: "from-lime-500 to-green-600",
  },
  {
    id: "welcome-onboard",
    name: "Welcome Onboarding",
    category: "Account",
    description: "Welcome message for new platform users",
    subject: "Welcome to Zelle via QuantumYield",
    previewText: "Your account is ready — get started today",
    icon: "PartyPopper",
    color: "from-yellow-400 to-orange-500",
  },
  {
    id: "account-suspended",
    name: "Account Suspended",
    category: "Account",
    description: "Notify user of a temporary account suspension",
    subject: "Your account has been temporarily suspended",
    previewText: "Action required to restore your access",
    icon: "Ban",
    color: "from-red-700 to-rose-800",
  },
  {
    id: "referral-bonus",
    name: "Referral Bonus",
    category: "Account",
    description: "Notify user about an earned referral reward",
    subject: "You've earned a referral bonus!",
    previewText: "Your referral reward has been credited",
    icon: "Gift",
    color: "from-fuchsia-500 to-pink-600",
  },
  {
    id: "kyc-verification",
    name: "KYC Verification",
    category: "Compliance",
    description: "Know Your Customer identity verification request",
    subject: "Action Required: Verify your identity",
    previewText: "Complete identity verification to continue",
    icon: "ScanFace",
    color: "from-violet-500 to-purple-600",
  },
  {
    id: "aml-hold",
    name: "AML Hold Notice",
    category: "Compliance",
    description: "Anti-money laundering transaction hold notice",
    subject: "Transaction hold applied — compliance review",
    previewText: "Your transfer is under regulatory review",
    icon: "ShieldAlert",
    color: "from-red-600 to-rose-700",
  },
  {
    id: "monthly-statement",
    name: "Monthly Statement",
    category: "Compliance",
    description: "Monthly e-Transfer activity summary",
    subject: "Your Zelle monthly statement is ready",
    previewText: "Review your monthly transaction history",
    icon: "ReceiptText",
    color: "from-slate-500 to-zinc-600",
  },
  {
    id: "large-transaction-review",
    name: "Large Transaction Review",
    category: "Compliance",
    description: "Additional verification for transfers above threshold",
    subject: "Large transaction — additional verification required",
    previewText: "Your transfer requires further review",
    icon: "ClipboardCheck",
    color: "from-orange-600 to-amber-700",
  },

  // Compliance & Settlement (34-39)
  {
    id: "settlement-confirmation",
    name: "Settlement Confirmation",
    category: "Settlement",
    description: "Confirm funds have been settled to the recipient account",
    subject: "Settlement confirmed — funds deposited to your account",
    previewText: "Your settlement of {{amount}} has been processed",
    icon: "BadgeCheck",
    color: "from-emerald-600 to-teal-700",
  },
  {
    id: "settlement-delayed",
    name: "Settlement Delayed",
    category: "Settlement",
    description: "Notify recipient of a delayed settlement with reason",
    subject: "Settlement delay — your funds are pending",
    previewText: "Your settlement has been temporarily delayed",
    icon: "CalendarClock",
    color: "from-amber-500 to-orange-600",
  },
  {
    id: "regulatory-hold",
    name: "Regulatory Hold",
    category: "Settlement",
    description: "FINTRAC or regulatory body hold placed on funds",
    subject: "Regulatory hold placed on your transaction",
    previewText: "Your funds are under regulatory review",
    icon: "ShieldAlert",
    color: "from-red-700 to-rose-800",
  },
  {
    id: "compliance-document-request",
    name: "Document Request",
    category: "Settlement",
    description: "Request for supporting compliance documents from recipient",
    subject: "Action required — compliance documents needed",
    previewText: "Please provide the requested documents to release your funds",
    icon: "FileText",
    color: "from-blue-600 to-indigo-700",
  },
  {
    id: "settlement-summary",
    name: "Settlement Summary",
    category: "Settlement",
    description: "End-of-day or period settlement summary report",
    subject: "Your settlement summary for {{period}}",
    previewText: "Review your complete settlement activity",
    icon: "ReceiptText",
    color: "from-slate-600 to-zinc-700",
  },
  {
    id: "dispute-resolution",
    name: "Dispute Resolution",
    category: "Settlement",
    description: "Update on a filed dispute and resolution outcome",
    subject: "Dispute update — case #{{transferId}}",
    previewText: "An update is available for your dispute case",
    icon: "ClipboardCheck",
    color: "from-violet-600 to-purple-700",
  },
]

export const templateCategories = [
  { id: "all", name: "All Templates", count: 39 },
  { id: "Transfer", name: "Transfer", count: 7 },
  { id: "Deposit", name: "Deposit", count: 6 },
  { id: "Security", name: "Security", count: 6 },
  { id: "Account", name: "Account", count: 7 },
  { id: "Request", name: "Request", count: 3 },
  { id: "Compliance", name: "Compliance", count: 4 },
  { id: "Settlement", name: "Settlement", count: 6 },
]

export function getTemplatesByCategory(category: string): EmailTemplate[] {
  if (category === "all") return emailTemplates
  return emailTemplates.filter((t) => t.category === category)
}

export function getTemplateById(id: string): EmailTemplate | undefined {
  return emailTemplates.find((t) => t.id === id)
}
