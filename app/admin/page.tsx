...
export default function AdminDashboard() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const reference = formData.reference || `ZLE-${Date.now().toString().slice(-6)}`
        ...
        <div className="container mx-auto px-4 py-4 md:py-6">
          <div className="flex items-center justify-between gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-[#6D1ED4] text-white rounded-lg font-semibold hover:bg-[#5A18B0] transition-colors focus:outline-none focus:ring-2 focus:ring-[#6D1ED4] focus:ring-offset-2"
            >
          ...
          <div className="lg:col-span-2">
            <Card className="shadow-lg border-2">
              <CardHeader>
                ...
                <CardDescription>Send instant USD payments via the Zelle network</CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="space-y-4 p-5 bg-muted/50 rounded-lg border">
                    ...
                    <div className="grid md:grid-cols-3 gap-4 pt-2">
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="institutionCode">Routing Prefix</Label>
                        <Input
                          id="institutionCode"
                          type="text"
                          placeholder="021"
                  ...
                  <Button
                    ...
                    className="w-full bg-[#6D1ED4] text-white hover:bg-[#5A18B0] text-lg py-6 font-bold"
                    size="lg"
                  >
                    {isSubmitting ? (
                      <>
              ...
              recentTransfers
                ...
                .map((transfer) => (
                  <div
                    key={transfer.id}
                    className="p-4 rounded-lg border-2 bg-card hover:bg-muted/50 transition-colors cursor-pointer group"
                    onClick={() => { setPreviewTransfer(transfer); setShowPendingTransfers(false) }}
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-sm truncate">{transfer.recipientName}</p>
